...
def _load_sets(cur, session_id: int) -> list[dict]:
    cur.execute(
        "SELECT * FROM session_sets WHERE session_id = ? ORDER BY set_number ASC, id ASC",
        (session_id,),
    )
    rows = cur.fetchall()
    for r in rows:
...
def _hydrate(cur, row: dict) -> dict:
    row["sets"] = _load_sets(cur, row["id"])
    return row
...
def list_sessions(
    ...
    with get_db() as conn:
        ...
        return [_hydrate(cur, r) for r in cur.fetchall()]
...
def log_set(session_id: int, req: SessionSetCreate, user_id: int = Depends(get_current_user_id)):
    with get_db() as conn:
        ...
        cur.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (session_id, req.exercise_id, req.set_number, req.reps, req.weight,
             req.duration_sec, req.distance_m, req.rpe,
             int(bool(req.completed if req.completed is not None else True)), req.notes),
        )
