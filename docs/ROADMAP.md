...
- [x] **Deploy-failure observability** — `fly-deploy.yml` now runs an `if: failure()` step that opens / comments on a single `[deploy] Backend deploy is failing` issue, mirroring the `backup-neon.yml` alert pattern. Surfaces the failed step name so operators can triage "build broke" vs "post-deploy smoke broke" without clicking into the run. The CI lint in #133 catches the *static* class; this catches the runtime class.
- [x] **Backend SHA in `/health/detailed`** — extends #127's frontend-SHA pattern so a single Settings tap can compare frontend SHA + backend SHA. Shipped in PR #212.
...
- [x] **Backup encryption-key rotation runbook** — `BACKUP_PASSPHRASE` is a single static secret. If it leaks, every retained backup is compromised. Rotation procedure documented in `docs/DISASTER_RECOVERY.md` § "Rotating `BACKUP_PASSPHRASE`".
